...
var campaignIds = {
    'default': "67607e5405f19f40b202a2d9",
}
var cookieDomain = "dariuslukas.com"
...
var initialSrc = "https://track.dariuslukas.com/" + campaignID + "?format=json" + "&referrer=" + referrer;
for (var i = 1; i <= 10; i++) {
...
if (!urlParams.get('rtkcid')) {
    ...
    rtkxhr.onreadystatechange = function () {
        if (rtkxhr.readyState == 4 && rtkxhr.status == 200) {
            ...
            if (sessionStorage.getItem('viewOnce') !== 1) {
                xhrr.open("GET", "https://track.dariuslukas.com/view?clickid=" + rawData.clickid + "&referrer=" + referrer)
                xhrr.send();
            }
    ...
    if (sessionStorage.getItem('rtkclickid') === 'undefined' || !sessionStorage.getItem('rtkclickid')) {
        ...
        xhrr = new XMLHttpRequest;
        if (sessionStorage.getItem('viewOnce') !== 1) {
            xhrr.open("GET", "https://track.dariuslukas.com/view?clickid=" + rtkClickID + "&referrer=" + referrer)
            xhrr.send();
        }
        if (registerViewOncePerSession) {
            sessionStorage.setItem('viewOnce', '1')
    ...
    if (sessionStorage.getItem('viewOnce') !== 1) {
        xhrTrack.open("GET", "https://track.dariuslukas.com/view?clickid=" + rtkClickID + "&referrer=" + referrer)
        xhrTrack.send();
    }
    if (registerViewOncePerSession) {
        sessionStorage.setItem('viewOnce', '1')
